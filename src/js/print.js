...
let printTypes = ['pdf', 'html', 'image', 'json', 'external-html'];
	...
	switch (printJS.params.type) {
		case 'pdf':
			...
			if (browser.isFirefox()) {
				...
				if (printJS.params.showModal) {
					printJS.disablePrintModal();
				}
			}
			else {
				printJS.pdf();
...
PrintJS.prototype.html = function (printElement) {
	// get HTML printable element
	printElement = printElement || document.getElementById(this.params.printable);
...
PrintJS.prototype.externalHtml = function () {
	let htmlURL = this.params.printable,
		blockId = 'printJS-HTMLBlock';

	// check if we received proper data
	if (typeof htmlURL !== 'string') {
		throw new Error('Invalid HTML page URL.');
	}

	let $body = $('body');
	let $wrapper = $body.find('#' + blockId);

	if (!$wrapper.length) {
		$wrapper = $('<div id="' + blockId + '"><div></div></div>').hide();
		$body.append($wrapper);
	}

	let $block = $wrapper.find('div');
